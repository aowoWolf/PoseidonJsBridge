...
myPlane.prototype.move = function(state){
    ...
    _this.planeStart = function (ev){
        _this.stage.addEventListener('touchmove',_this.planeMove , false);
        _this.stage.addEventListener('touchend', function(ev){
            _this.stage.removeEventListener('touchmove',_this.planeMove , false);
        }, false);
    }

    _this.planeMove = function (ev){
        ev.preventDefault();
        var E = ev.touches[0];
    ...
    _this.stage.addEventListener('touchstart', _this.planeStart, false);
};
//飞机停止跟随鼠标
myPlane.prototype.stop = function(){
    this.stage.removeEventListener('touchstart',this.planeStart , false);
    this.stage.removeEventListener('touchmove',this.planeMove , false);
